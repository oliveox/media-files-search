        ...
        try {
            ...
            for (let filePath of filePaths) {
                ...
                try {
                    ...
                    // extension = extAndMime ? extAndMime.ext : MediaType[MediaType.NOT_SUPPORTED];
                    extension = extAndMime ? extAndMime.ext : GeneralUtilsService.fileTypeEnumToString(FManager.FileType.NOT_SUPPORTED);
                } catch (err) {
                    extension = GeneralUtilsService.fileTypeEnumToString(FManager.FileType.NOT_SUPPORTED);
                }
            ...
            if (writeToFile) {
                ...
                fs.writeFile(fileName, JSON.stringify(filesByExtension), (err: Error) => {
                    if (err) throw err;
            ...
            files = await Promise.all(
                files.map(async (file: string) => {
                        ...
                        try {
                            ...
                            logger.warn(`Error reading filysystem stat for [${filePath}]`)
                            return;
                        }
                }));
            return files;
